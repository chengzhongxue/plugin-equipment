...
group: 图片分组名称, 对应 [#EquipmentGroupVo](#equipmentgroupvo).metadata.name
...
<th:block th:each="group : ${equipmentFinder.groupBy()}">
    ...
    <ul>
        <li th:each="equipment : ${group.equipments}">
            <img th:src="${equipment.spec.cover}" th:alt="${equipment.spec.displayName}" width="280">
        </li>
    </ul>
</th:block>
```
...
| 对应模型   | group            | kind       |
| ---------- | ---------------- | ---------- |
| 装备       | equipment.kunkunyu.com | Equipment       |
| 装备分组 | equipment.kunkunyu.com | EquipmentGroup |
